...
class UsuarioController extends Controller
{
	...
	public function actionJugadoresByJuego($id_juego)
	{
		...
		$sql="SELECT * FROM usuario u WHERE EXISTS (SELECT * FROM juego j WHERE j.id_juego='".$id_juego."' AND u.id=j.id_jugador AND u.perfil_publico=1 AND u.id<>'".$id_usuario."')";
	...
	*/
	...
	public function solicitarAmistad($id_jugador, $id_amigo)
	{
		$connection=Yii::app()->db;
		$sql='SELECT id FROM amistad WHERE id_jugador='.$id_jugador;
		$dataReader=$connection->createCommand($sql)->query();
		if($dataReader->read()==false)
		{
			$connection=Yii::app()->db;
					
			$sql="INSERT INTO amistad (id, id_jugador, id_amigo, estado, fecha) VALUES(NULL, :id_jugador, :id_amigo, 'pendiente', CURRENT_TIMESTAMP)";
			$command=$connection->createCommand($sql);
			$command->bindParam(":id_jugador",$id_jugador,PDO::PARAM_STR);
			$command->bindParam(":id_amigo",$id_amigo,PDO::PARAM_STR);
			$command->execute();
		}
	
	}
	
	public function actionAjaxAceptarAmistad()
	{
	  ...
	  //
	  sleep(5);
	  $model=Amistad::model()->findByPk($id_solicitud);
	  $model->estado=('aceptada');
	...
	public function actionAjaxRechazarAmistad()
	{
	  $id_solicitud = $_POST['id'];
	  //$id_jugador=Yii::app()->user->id;
